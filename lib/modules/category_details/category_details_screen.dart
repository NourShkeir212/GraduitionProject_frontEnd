...
class CategoryDetailsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<AppThemeCubit, AppThemeStates>(
      ...
      return BlocProvider(
        create: (context) => AppCategoryDetailsCubit()..getCategoryDetails(categoryData['id'].toString()),
        child: BlocConsumer<AppCategoryDetailsCubit, AppCategoryDetailsStates>(
            ...
            if (state is AppCategoryDetailsAddToFavoritesSuccessState) {
              successSnackBar(
                isDark: isDark,
                  context: context,
                  message:
                      'Successfully added to favorites'.translate(context));
            }
            if (state is AppCategoryDetailsDeleteFromFavoritesSuccessState) {
              successSnackBar(
                isDark: isDark,
                  context: context,
                  message:
                      'Successfully removed from favorites'.translate(context));
            }
            if (kDebugMode) {
              print('object');
            }
          ...
          builder: (context, state) {
            ...
            return Scaffold(
                appBar:
                    myAppBar(title: appBarTitle.translate(context), actions: [
                  const MyAppBarLogo(),
                ]),
                body: SafeArea(
                        ...
                        isDark: isDark,
                                  ...
                                  child: SingleChildScrollView(
                                    ...
                                    child: Column(
                                      children: [
                                        if (state
                                            is AppCategoryDetailsAddToFavoritesLoadingState)
                                          const LinearProgressIndicator(),
                                        if (state
                                            is AppCategoryDetailsDeleteFromFavoritesLoadingState)
                                          const LinearProgressIndicator(),
                                        Padding(
                                                          ...
                                                          await Navigator.push(
                                                              ...
                                                              MaterialPageRoute(
                                                                builder: (context) => WorkerScreen(
                                                                  workerId: cubit.workersData!.data![index].id.toString(),),
                                                              ),
                                                          );
                                                      if (response == 'true') {
                                                        cubit.getCategoryDetails(categoryData['id']);
                                                      } else {}
                                                    },
                                                    isFavorites: false, data: cubit.workersData!.data![index],
                                                    favIconCondition: favorites[cubit.workersData!.data![index].id!] == true,
                                                    onFavoritePressed: () {cubit.favoritesFunction(id: cubit.workersData!.data![index].id!);
                                                    },
                                                  );
                                                },
